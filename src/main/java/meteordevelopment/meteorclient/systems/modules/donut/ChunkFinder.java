...
public class ChunkFinder extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        if (mc.world != null && mc.world.getDimension() != lastDimension) onActivate();
    }
    ...
    private void onPacket(PacketEvent.Receive event) {
        if (event.packet instanceof BlockUpdateS2CPacket packet) {
            checkFlowUpdate(packet.getPos(), packet.getState());
        } else if (event.packet instanceof ChunkDeltaUpdateS2CPacket packet) {
            packet.visitUpdates(this::checkFlowUpdate);
        }
    }
    ...
    private static boolean hasFlowingLiquid(WorldChunk chunk) {
        for (ChunkSection section : chunk.getSectionArray()) {
            if (section == null || section.isEmpty() || !section.hasAny(ChunkFinder::isFlowing)) continue;
    ...
    private static boolean isFlowing(BlockState state) {
        return !state.getFluidState().isEmpty() && !state.getFluidState().isStill();
    }
