...
public class TunnelBaseFinder extends Module {
    ...
    private void queueScan(WorldChunk chunk) {
        ...
        worker.submit(() -> {
            ...
            synchronized (lock) {
                ...
                fresh = update(chunk.getPos().toLong(), runs, needed);
            }
    ...
    private List<Run> scan(WorldChunk chunk, int needed, int maxHeight) {
        List<Run> runs = new ArrayList<>();

        int baseX = chunk.getPos().getStartX();
        int baseZ = chunk.getPos().getStartZ();
        int bottom = chunk.getBottomY() + 1;
        int top = Math.min(maxHeight, chunk.getBottomY() + chunk.getHeight() - 3);
    ...
    private static boolean isTunnelCell(WorldChunk chunk, BlockPos.Mutable pos, int x, int y, int z, boolean alongX) {
