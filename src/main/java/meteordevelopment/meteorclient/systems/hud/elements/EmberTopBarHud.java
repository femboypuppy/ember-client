...
public class EmberTopBarHud extends HudElement {
    ...
    public void render(HudRenderer renderer) {
        ...
        if (showUser.get()) {
            parts.add(mc.getSession().getUsername());
            colors.add(white);
        }
        ...
        if (showDurability.get() && mc.player != null) {
            ...
            if (held.isDamageable()) {
                parts.add((held.getMaxDamage() - held.getDamage()) + " dur");
                colors.add(white);
            }
        }

        if (showServer.get()) {
            parts.add(mc.getCurrentServerEntry() != null ? mc.getCurrentServerEntry().address : "Singleplayer");
            colors.add(gray);
        }
        ...
        if (glow.get()) renderer.softGlow(x, y, total, height, radius, 10 * s, new Color(accent.r, accent.g, accent.b, 190));
