...
public class EmberSettingsPopup {
    ...
    private Color accentAlpha(int a) {
        Color c = accent();
        return new Color(c.r, c.g, c.b, MathHelper.clamp(a, 0, 255));
    }
    ...
    public void render(GuiRenderer r, double mouseX, double mouseY, float delta) {
        ...
        text("Right-click a setting to reset it   Â·   Esc to close", x + w / 2, bodyBottom + (FOOTER_H - 9) / 2,
     ...
     */
    public void renderText(DrawContext graphics) {
        if (target == null) return;
        flushText(graphics);
    }
    ...
    private void flushText(DrawContext graphics) {
        ...
        for (Map.Entry<Double, List<TextOp>> entry : byScale.entrySet()) {
            theme.textRenderer().begin(theme.scale(entry.getKey()));
    ...
    private double sliderPct(Row row) {
        ...
        if (row.setting() instanceof IntSetting is) {
            ...
            return hi <= lo ? 0.5 : MathHelper.clamp((double) (is.get() - lo) / (hi - lo), 0, 1);
        }
        if (row.setting() instanceof DoubleSetting ds) {
            ...
            return MathHelper.clamp((ds.get() - lo) / (hi - lo), 0, 1);
        }
        return 0;
    }
    ...
    private String sliderText(Row row) {
        ...
        if (row.setting() instanceof DoubleSetting ds) {
            ...
            return String.format("%." + MathHelper.clamp(ds.decimalPlaces, 0, 2) + "f", v);
        }
        return "";
    }
    ...
    private void applySlider(Row row, Hit hit, double px) {
        ...
        double pct = MathHelper.clamp((px - trackX) / trackW, 0, 1);
